...
def main():
    ...
    if page == "Temperature Map":
        ...
        if uploaded_file is not None:
            ...
            # Check for available FIPS column and process accordingly
            if 'fips_05' in data.columns:
                # Process county data
                data['fips_05'] = data['fips_05'].astype(str).str.strip().str.replace('"', '').str.zfill(5)
                counties = data['fips_05'].unique()
                counties = sorted(counties)  # Sort for better UX
                selected_fips_column = 'fips_05'
            elif 'fips' in data.columns:
                # Process state data
                data['fips'] = data['fips'].astype(str).str.strip().str.zfill(2)  # For state-level FIPS, it's usually 2 digits
                counties = data['fips'].unique()
                counties = sorted(counties)  # Sort for better UX
                selected_fips_column = 'fips'
            else:
                st.error("FIPS column not found in the dataset!")
                return
            ...
            filtered_data_copy['year'] = pd.to_datetime(filtered_data_copy['year'], format='%Y')  # Ensure year is datetime
